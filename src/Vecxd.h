...
template <typename T> class MultiArray
{
  public:
    MultiArray() = default;
    MultiArray(const std::initializer_list<std::size_t> &dims) : dims_(dims), data_(CalculateLinearDimension(dims_))
    {
    }
  ...
  private:
    ...
    std::vector<T> data_;
    static std::size_t CalculateLinearDimension(const std::vector<std::size_t> &dims)
    {
        size_t result = 1;
    ...
    [[nodiscard]] std::size_t index(const std::vector<std::size_t> &indices) const
    {
